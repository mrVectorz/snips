...
for file in ${files[@]}; do
  awk -v COUNT=$pcount '{if($0 ~ /^\w+orkers *= */) print gensub(/(\w* *= *)[0-9]+$/, "\\1"COUNT, "g", $0); else print}' < $file > ${file}.bak && cp -f ${file}.bak $file
done
...
# validations
echo "Everything should set to have $pcount worker/process"
for file in ${files[@]}; do
  echo $file
  egrep '^\w+orkers *= *' $file
done
